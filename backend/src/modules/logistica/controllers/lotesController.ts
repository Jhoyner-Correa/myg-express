  ...
  try {
    ...
    const [result]: any = await pool.query(
      `INSERT INTO lotes_carga (sede_id, id_usuario_creador, fecha, zona, nombre_lote, estado)
       VALUES (?, ?, ?, ?, ?, 'pendiente')`,
      [sede_id, usuario_id, businessDate(), nombreBase, routeName]
    );
    ...
    const [rows]: any = await pool.query(
      isSuper
        ? `SELECT 
          ...
          FROM lotes_carga l
          INNER JOIN sedes s ON l.sede_id = s.id
          WHERE l.id = ? AND l.fecha_eliminacion IS NULL
          LIMIT 1`
        : `SELECT 
            l.id,
            l.sede_id,
