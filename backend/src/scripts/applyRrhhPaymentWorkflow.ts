...
const MIGRATION_ID = '036_rrhh_payment_workflow';
const LOCK_NAME = 'myg_rrhh_payment_workflow';
...
async function main() {
  ...
  if (Number(lock?.acquired) !== 1) throw new Error('No se pudo adquirir el candado del workflow de pagos.');
  try {
    const [applied] = await pool.query<RowDataPacket[]>('SELECT id FROM schema_migrations WHERE id = ?', [MIGRATION_ID]);
    if (applied.length) return console.log('Migracion 036 ya estaba aplicada.');
    ...
    console.log('Migracion 036 del workflow de pagos completada.');
  } finally {
