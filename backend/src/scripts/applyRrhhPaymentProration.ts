...
const MIGRATION_ID = '040_rrhh_payment_proration';
const LOCK_NAME = 'myg_rrhh_payment_proration';
...
async function main() {
  ...
  if (Number(lock?.acquired) !== 1) throw new Error('No se pudo adquirir el candado de prorrateo de pagos.');
  try {
    const [applied] = await pool.query<RowDataPacket[]>('SELECT id FROM schema_migrations WHERE id = ?', [MIGRATION_ID]);
    if (applied.length) return console.log('Migracion 040 ya estaba aplicada.');
    ...
    console.log('Migracion 040 de prorrateo de pagos completada.');
  } finally {
