...
const MIGRATION_ID = '018_rrhh_weekly_scope';
const LOCK_NAME = 'myg_rrhh_weekly_scope';
...
function statements() {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '018_rrhh_weekly_scope.sql'),
    'utf8',
...
async function assertPreflight() {
  const checks: Array<[string, string]> = [
    ['asignaciones huerfanas', `SELECT COUNT(*) total FROM personal_empleado_horarios assignment
    ...
    ['dias semanales invalidos', `SELECT COUNT(*) total FROM personal_empleado_horarios
      WHERE dia_semana NOT BETWEEN 1 AND 7`],
    ['periodos semanales invalidos', `SELECT COUNT(*) total FROM personal_empleado_horarios
...
async function main() {
  ...
  if (Number(lock?.acquired) !== 1) throw new Error('No se pudo adquirir el candado de migracion semanal.');
  try {
    const [applied] = await pool.query<RowDataPacket[]>('SELECT id FROM schema_migrations WHERE id = ?', [MIGRATION_ID]);
    if (applied.length) {
      console.log('Migracion 018 ya estaba aplicada.');
      return;
    }
    await assertPreflight();
    for (const statement of statements()) await pool.query(statement);
    await pool.query('INSERT INTO schema_migrations (id) VALUES (?)', [MIGRATION_ID]);
    console.log('Migracion 018 de jerarquia semanal completada.');
  } finally {
