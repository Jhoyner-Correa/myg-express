...
const MIGRATION_ID = '022_user_profile_photos';
const LOCK_NAME = 'myg_user_profile_photos';

type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '022_user_profile_photos.sql'),
    'utf8',
