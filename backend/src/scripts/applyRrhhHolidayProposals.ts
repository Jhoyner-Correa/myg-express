...
const MIGRATION_ID = '028_rrhh_holiday_proposals';
const LOCK_NAME = 'myg_rrhh_holiday_proposals';
type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '028_rrhh_holiday_proposals.sql'),
    'utf8',
...
async function main() {
  ...
  if (Number(lock?.acquired) !== 1) throw new Error('No se pudo bloquear la migración de propuestas de feriados.');
  try {
    const [applied] = await pool.query<RowDataPacket[]>('SELECT id FROM schema_migrations WHERE id = ?', [MIGRATION_ID]);
    if (applied.length) {
      console.log('Migración 028 ya estaba aplicada.');
      return;
    }
    for (const statement of statements()) await pool.query(statement);
    await pool.query('INSERT INTO schema_migrations (id) VALUES (?)', [MIGRATION_ID]);
    console.log('Migración 028 de propuestas de feriados completada.');
  } finally {
