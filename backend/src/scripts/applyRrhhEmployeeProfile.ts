...
const MIGRATION_ID = '020_rrhh_employee_profile';
const LOCK_NAME = 'myg_rrhh_employee_profile';

type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '020_rrhh_employee_profile.sql'),
    'utf8',
