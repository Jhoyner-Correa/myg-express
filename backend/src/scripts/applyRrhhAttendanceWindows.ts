...
const MIGRATION_ID = '023_rrhh_attendance_windows';
const LOCK_NAME = 'myg_rrhh_attendance_windows';
type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '023_rrhh_attendance_windows.sql'),
    'utf8',
