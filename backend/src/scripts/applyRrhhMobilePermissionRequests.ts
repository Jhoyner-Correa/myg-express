...
const MIGRATION_ID = '031_rrhh_mobile_permission_requests';
const LOCK_NAME = 'myg_rrhh_mobile_permission_requests';
...
async function main() {
  ...
  try {
    ...
    if (applied.length) {
      console.log('Migracion 031 ya estaba aplicada.');
      return;
    }
    const source = fs.readFileSync(
      path.resolve(process.cwd(), 'migrations', '031_rrhh_mobile_permission_requests.sql'),
      'utf8',
    ...
    console.log('Migracion 031 de solicitudes moviles completada.');
  } finally {
