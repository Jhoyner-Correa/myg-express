...
const MIGRATION_ID = '019_rrhh_employee_codes';
const LOCK_NAME = 'myg_rrhh_employee_codes';

type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '019_rrhh_employee_codes.sql'),
    'utf8',
...
async function main(): Promise<void> {
  ...
  if (Number(lock?.acquired) !== 1) throw new Error('No se pudo adquirir el candado de codigos de colaboradores.');
  try {
    const [applied] = await pool.query<RowDataPacket[]>('SELECT id FROM schema_migrations WHERE id = ?', [MIGRATION_ID]);
    if (applied.length) {
      console.log('Migracion 019 ya estaba aplicada.');
      return;
    }
    for (const statement of statements()) await pool.query(statement);
    await pool.query('INSERT INTO schema_migrations (id) VALUES (?)', [MIGRATION_ID]);
    console.log('Migracion 019 de codigos automaticos completada.');
  } finally {
