...
const MIGRATION_ID = '024_rrhh_overtime_segments';
const LOCK_NAME = 'myg_rrhh_overtime_segments';
type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '024_rrhh_overtime_segments.sql'),
    'utf8',
