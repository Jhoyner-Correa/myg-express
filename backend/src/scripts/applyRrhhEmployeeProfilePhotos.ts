...
const MIGRATION_ID = '021_rrhh_employee_profile_photos';
const LOCK_NAME = 'myg_rrhh_employee_profile_photos';

type LockRow = RowDataPacket & { acquired: number };

function statements(): string[] {
  const source = fs.readFileSync(
    path.resolve(process.cwd(), 'migrations', '021_rrhh_employee_profile_photos.sql'),
    'utf8',
