...
const MIGRATION_ID = '034_rrhh_attendance_justifications';
const LOCK_NAME = 'myg_rrhh_attendance_justifications';
...
async function main() {
  ...
  try {
    ...
    if (applied.length) {
      console.log('Migracion 034 ya estaba aplicada.');
      return;
    }
    const source = fs.readFileSync(
      path.resolve(process.cwd(), 'migrations', '034_rrhh_attendance_justifications.sql'),
      'utf8',
    ...
    console.log('Migracion 034 de justificaciones de asistencia completada.');
  } finally {
