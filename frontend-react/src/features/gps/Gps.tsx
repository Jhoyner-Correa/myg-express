...
export function Gps() {
  ...
  return <main className={`main ${styles.page}`} id="main-content">
    <PageHeader icon={<MapPinned />} title="Rastreo GPS" subtitle="Ubicación operativa y recorridos del personal autorizado" metadata={selectedSite?.name ?? (canViewAllSites ? 'Todas las sedes' : user?.sede_nombre ?? 'Sede operativa')} tone="corporate" />
    <section className={styles.content}>
      <div className={styles.toolbar}>
        <div><span>Operación en vivo</span><strong>Monitoreo de personal</strong></div>
        <label className={styles.sitePicker}><MapPin /><select aria-label="Alcance del mapa" value={siteId ?? 'all'} onChange={event => setSiteId(event.target.value === 'all' ? null : Number(event.target.value))}>{canViewAllSites && <option value="all">Todas las sedes</option>}{sites.map(site => <option key={site.id} value={site.id}>{site.name}</option>)}</select></label>
      </div>
