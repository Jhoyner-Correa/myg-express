...
export function AttendancePanel({ siteId, canManage }: { siteId: number | null; canManage: boolean }) {
  ...
  const employees = useMemo(() => {
    ...
    return (dashboard?.employees ?? []).filter(employee => {
      const matchesText = !term || `${employee.names} ${employee.last_names} ${employee.employee_code} ${employee.job_role} ${employee.site_name}`.toLocaleLowerCase('es').includes(term);
  ...
  return <div className={styles.attendanceStack}>
    ...
    <article className={styles.card}>
      ...
      {loading && !dashboard ? <PageLoader compact label="Consultando asistencia" /> : error ? <div className={styles.tableError} role="alert"><p>{getApiErrorMessage(error, 'No se pudo consultar la asistencia.')}</p><Button size="sm" variant="secondary" onClick={() => void load()}>Reintentar</Button></div> : <div className={styles.tableWrap}><table className={`${styles.table} ${styles.attendanceTable}`}><thead><tr><th>Colaborador</th>{siteId === null && <th>Sede</th>}<th>Horario</th><th>Entrada</th><th>Salida almuerzo</th><th>Regreso</th><th>Salida final</th><th>Estado</th><th>Tardanza</th><th>Extra</th>{canManage && <th aria-label="Acciones" />}</tr></thead><tbody>
        {employees.map(employee => <tr key={employee.employee_id}><td><div className={styles.person}><span>{employee.names.charAt(0)}{employee.last_names.charAt(0)}</span><div><strong>{employee.names} {employee.last_names}</strong><small>{employee.job_role} · {employee.employee_code}</small></div></div></td>{siteId === null && <td>{employee.site_name}</td>}<td>{employee.schedule ? <div className={styles.scheduleCell}><strong>{employee.schedule.name}</strong><small>{employee.schedule.start_time.slice(0, 5)}–{employee.schedule.end_time.slice(0, 5)}</small></div> : <span className={styles.muted}>Sin asignar</span>}</td><td className={styles.clockCell}>{clock(employee.marks.entry)}</td><td className={styles.clockCell}>{clock(employee.marks.lunch_out)}</td><td className={styles.clockCell}>{clock(employee.marks.lunch_return)}</td><td className={styles.clockCell}>{clock(employee.marks.exit)}</td><td><span className={`${styles.attendanceStatus} ${styles[`attendance${employee.status}`]}`}><i />{STATUS_LABELS[employee.status]}</span></td><td>{employee.delay_minutes ? `${employee.delay_minutes} min` : '—'}</td><td>{employee.overtime_minutes ? `${employee.overtime_minutes} min` : '—'}</td>{canManage && <td><div className={styles.actions}><button title="Corregir asistencia" aria-label={`Corregir asistencia de ${employee.names}`} onClick={() => setCorrecting(employee)}><PencilLine /></button></div></td>}</tr>)}
        {!employees.length && <tr><td colSpan={(canManage ? 10 : 9) + (siteId === null ? 1 : 0)}><div className={styles.empty}>No hay colaboradores que coincidan con los filtros seleccionados.</div></td></tr>}
      </tbody></table></div>}
    </article>
