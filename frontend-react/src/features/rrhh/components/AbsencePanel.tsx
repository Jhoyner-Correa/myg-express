          ...
          <tbody>{visibleItems.map((item) => {
            ...
            return <tr key={item.key} className={item.pending ? styles.pendingRow : ''}>
              ...
              <td><div className={styles.rowActions}>
                ...
                {canManage && item.pending && <><button type="button" className={styles.approveAction} aria-label={`${item.kind === 'JUSTIFICACION' ? 'Justificar sin descuento' : 'Aprobar solicitud'} de ${item.employeeName}`} data-tooltip={item.kind === 'JUSTIFICACION' ? 'Justificar · sin descuento' : 'Aprobar'} onClick={() => openResolution(item, 'APPROVE')}><Check /></button><button type="button" className={styles.rejectAction} aria-label={`${item.kind === 'JUSTIFICACION' ? 'Confirmar falta' : 'Rechazar solicitud'} de ${item.employeeName}`} data-tooltip={item.kind === 'JUSTIFICACION' ? 'Confirmar falta · descontar' : 'Rechazar'} onClick={() => openResolution(item, 'REJECT')}><X /></button></>}
